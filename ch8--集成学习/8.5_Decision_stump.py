...
def decision_sdumps_MaxInfoGain(X, Y):
    ...
    for i in range(n):  # 遍历各个候选特征
        ...
        #如果样本正类>=负类，则sum(Y[x <= best_t])>=0,那么这个表达式为真，其值为true,*2-1为1，否则为加*2-1为-1。这样就实现了取多数类的类别
...
for t in range(T):
    ...
    # 计算并存储训练误差
    df, t, lv, rv = h  # 基学习器参数
    #因为(X[:, df] <= t)表示的是最优划分特征的左分支，且这是一个bool数组，因此乘以其类别（1，-1）后，false的为0，即非最优特征做分支
    #而true，也就是最优划分特征左分支，则会为其类别负类就是-1，而正类就是1，两个向量相加就是最后的预测结果
